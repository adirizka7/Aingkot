...
$querySopir = $DB->query("SELECT * FROM sopir o JOIN carter_angkot c ON o.id_sopir=c.id_sopir WHERE id_pelanggan=".$_SESSION['userSession']);
$rowinfosupir = array();
while($row = mysql_fetch_assoc($querySopir)) {
    $rowinfosupir[] = $row;
}
$querysemuasopir = $DB->query("SELECT * FROM sopir o JOIN carter_angkot c ON o.id_sopir=c.id_sopir");
$rowsupir = array();
while($row = mysql_fetch_assoc($querySopir)) {
    $rowsupir[] = $row;
}
	...
	<ons-template id="tab3.php">
		<ons-page id = "third-page">
          ...
          <?php if($_SESSION['status']==1) {
              ...
              <span class="list-item__subtitle"><?php echo $rowsupir[$i]['available'];?></span>
							<?php if($rowsupir[$i]['no_plat']!="") {?><span class="list-item__subtitle"><?php echo $rowsupir[$i]['no_plat'];?></span><?php }?>
              <span class="list-item__subtitle"><?php echo $rowsupir[$i]['no_telp'];?></span>
			      </div>
						<div class="right">
